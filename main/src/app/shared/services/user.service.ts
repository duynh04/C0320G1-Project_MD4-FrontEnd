...
export class UserService {
  private readonly API_URL_USER = "http://localhost:8080/api/v1";
  ...
  comparePassword(c: AbstractControl) {
   ...
   getAllUser(): Observable<User[]> {
    return this.http.get<User[]>(this.API_URL_USER+"/userList");
  }

  createUser(user: User): Observable<User> {
    return this.http.post<User>(this.API_URL_USER + "/create", user);
